    ...
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    data.append({
                        "date": datetime.datetime.strptime(row["date"].strip(), "%Y-%m-%d").date(),
...
def generate_chart():
    ...
    line, = ax.plot(dates, valeurs, color='#00ADB5', linewidth=3, marker='o', 
                    markersize=8, markerfacecolor='#EEEEEE', markeredgecolor='#00ADB5',
                    markeredgewidth=2, label="Patrimoine")
    
    ax.fill_between(dates, valeurs, min(valeurs) * 0.98 if min(valeurs) > 0 else 0, color='#00ADB5', alpha=0.15)
    
    if config:
        goal = config["objectif"]
        end_date = datetime.datetime.strptime(config["date_fin"], "%Y-%m-%d").date()
        ax.axhline(y=goal, color='#FF5722', linestyle='--', linewidth=1.5, alpha=0.8, label=f"Objectif ({goal:,.0f} €)")
        all_dates = dates + [end_date]
        ax.set_xlim(min(all_dates) - datetime.timedelta(days=2), max(all_dates) + datetime.timedelta(days=2))
        ax.plot(end_date, goal, marker='*', markersize=12, color='#FF5722', markeredgecolor='#EEEEEE', label="Date Cible")
    
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%d %b'))
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=max(1, (max(dates) - min(dates)).days // 6) if len(dates) > 1 else 1))
    ...
    for i, (date, val, note) in enumerate(zip(dates, valeurs, notes)):
        if note:
            ...
            ax.annotate(
                ...
                color='#EEEEEE',
                ha='center',
                bbox=dict(boxstyle="round,pad=0.3", fc="#252525", ec="#333333", alpha=0.8)
            )
    ...
    if config:
        ...
        stat_text = f"Actuel : {latest:,.2f} €  ({sign}{diff:,.2f} € | {sign}{pct:.2f}%)"
    ...
    ax.legend(loc='upper left', framealpha=0.6, facecolor='#1e1e1e', edgecolor='#333333')
...
def get_data():
    ...
    return jsonify({
        "config": config,
        "entries": [
            {"date": e["date"].strftime("%Y-%m-%d"), "valeur": e["valeur"], "note": e["note"]}
            for e in entries
        ],
        "stats": {
            "latest": latest,
...
def ensure_server_running():
    ...
    with flask_lock:
        if not FLASK_STARTED:
            server_thread = threading.Thread(target=lambda: app.run(host="127.0.0.1", port=5005, debug=False), daemon=True)
            server_thread.start()
            FLASK_STARTED = True
...
def run_cli():
    ...
    if not config:
        ...
        while True:
            date_str = input("Date de fin [AAAA-MM-JJ] (défaut: 2026-09-30) : ").strip() or "2026-09-30"
            try:
                datetime.datetime.strptime(date_str, "%Y-%m-%d")
                break
            except ValueError:
    ...
    while True:
        ...
        print(f"🎯 Objectif : {goal:,.2f} € d'ici le {config['date_fin']} (Progression: {progress_pct:.1f}%)")
        print("-" * 50)
        ...
        if choice == "1":
            ...
            date_str = input(f"Date [AAAA-MM-JJ] (défaut: {default_date}) : ").strip() or default_date
            try:
                datetime.datetime.strptime(date_str, "%Y-%m-%d")
            except ValueError:
            ...
            for idx, entry in enumerate(data):
                print(f"{idx + 1}. {entry['date'].strftime('%Y-%m-%d')} — {entry['valeur']:,.2f} € ({entry['note']})")
                
            del_choice = input("Entrez le numéro à supprimer (ou Entrée pour annuler) : ").strip()
            if not del_choice:
                continue
            ...
            print("-" * 58)
            for entry in data:
                print(f"{entry['date'].strftime('%Y-%m-%d'):<12} | {entry['valeur']:>12,.2f} € | {entry['note']:<25}")
...
class PatrimoineApp(rumps.App):
    ...
    def update_menu(self):
        ...
        if not config:
            ...
            self.menu.clear()
            self.menu.add(rumps.MenuItem("⚙️ Configurer l'Objectif (Web)", callback=self.on_open_gui))
        ...
        self.menu.clear()
        self.menu.add(rumps.MenuItem(f"Patrimoine Actuel : {latest:,.2f} €", callback=None))
        self.menu.add(rumps.MenuItem(f"Objectif : {config['objectif']:,.2f} € ({latest / config['objectif'] * 100:.1f}%)", callback=None))
        self.menu.add(rumps.MenuItem(f"Cible : {config['date_fin']}", callback=None))
        self.menu.add(rumps.separator)
        
        self.menu.add(rumps.MenuItem("🖥️ Ouvrir l'interface Web", callback=self.on_open_gui))
        self.menu.add(rumps.MenuItem("📈 Afficher le graphique (PNG)", callback=self.on_show_chart))
        self.menu.add(rumps.MenuItem("➕ Ajouter une entrée (Rapide)...", callback=self.on_add_entry))
        self.menu.add(rumps.MenuItem("📋 Voir l'historique (Texte)", callback=self.on_view_history))
        self.menu.add(rumps.MenuItem("⚙️ Modifier l'Objectif", callback=self.on_reset_config))
        self.menu.add(rumps.MenuItem("📂 Ouvrir le fichier CSV", callback=self.on_open_csv))
    ...
    def on_add_entry(self, _):
        ...
        if response.clicked == 1:
            ...
            self.update_menu()
            rumps.notification("Succès", "Patrimoine mis à jour", f"Nouveau solde: {valeur:,.2f} €")
    ...
    def on_view_history(self, _):
        ...
        for entry in data:
            history_lines.append(f"{entry['date'].strftime('%Y-%m-%d')} | {entry['valeur']:>10,.2f} € | {entry['note']}")
...
def main():
    if len(sys.argv) > 1:
        if sys.argv[1] in ["--cli", "-c"]:
            ...
            print("Arguments valides: --cli pour la console, --gui pour l'interface web.")
    else:
